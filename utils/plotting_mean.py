...
def plot_volumes_batch(env_history, confidence=0.5, alpha=0.5):
    ...
    for commodity in range(total_h.shape[1]):
        plt.plot(total_mean[:, commodity], label=f'Товар {commodity + 1}', )
        plt.fill_between(
            range(total.shape[0]),
...
def plot_environment_batch(env_history, confidence=0.5, alpha=0.5, window_size=5):
    data = env_history
    ...
    def plot_data(ax, data, item_index, title, logscale=False):
        ...
        for firm_index in range(num_firms):
            data_ma = np.convolve(
                data_mean[firm_index, item_index],
                np.ones(window_size) / window_size,
                mode='full')[:-window_size+1]
    ...
    for i in range(finances_mean.shape[1]):
        data_ma = np.convolve(finances_mean[:, i],
                              np.ones(window_size) / window_size,
                              mode='full')[:-window_size+1]
        ax[0].plot(periods,
                   data_ma,
    ...
    if ncols == 2:
        ...
        for i in range(limits.shape[0]):
            ax[1].plot(periods, limits[i].flatten(), label=labels[i], color=colors[i])
            ax[1].fill_between(
                periods,
