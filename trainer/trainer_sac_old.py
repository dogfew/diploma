        ...
        for idx in range(max_episode_length):
            ...
            for firm_id in range(self.n_agents):
                ...
                with torch.no_grad():
                    ...
                    next_q_values1 = self.q_critic_target(x_next, actions_next)[:, firm_id]
                    next_q_values2 = self.q_critic2_target(x_next, actions_next)[:, firm_id]
                    next_v = torch.minimum(
                        next_q_values1, next_q_values2
                    ) - self.entropy_reg * log_probs_next[:, firm_id].mean(dim=-1)
                    q_values_target = (
